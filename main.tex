...
% For clickable links
\usepackage{hyperref}
% descriptions without indentation after newline
\usepackage{enumitem}
% For proper hyphenation etc.
\usepackage[german]{babel}

% für Kodierungs-Zeichnungen
\usepackage{pgfplots}
...
\begin{tikzpicture}
    ...
    \draw[line width=0.5mm, smooth, samples=100, domain=0:0.4*\PI] plot (\x,{0.3*sin(\x*10 r)-4.8});
    \draw[line width=0.5mm, smooth, samples=100, domain=0.5*\PI:0.9*\PI] plot (\x-\PI*0.1,{0.3*sin(\x*10 r)-4.8});
    \draw[line width=0.5mm, smooth, samples=100, domain=0.8*\PI:1.6*\PI] plot (\x,{0.3*sin(\x*10 r)-4.8});
    \draw[line width=0.5mm, smooth, samples=100, domain=1.7*\PI:2.1*\PI] plot (\x-\PI*0.1,{0.3*sin(\x*10 r)-4.8});
...
\begin{tikzpicture}[
        ...
        \draw[dashed] (4.75,0.15)--(4.75,-1.2);
...
\begin{minipage}{0.5\textwidth}
    In der Mitte einer jeden Zeiteinheit findet eine Pegeländerung statt. Der Datenstrom wird wie folgt kodiert:
    \begin{description}[style=unboxed,leftmargin=0cm]
...
\begin{minipage}{0.45\textwidth}
    \begin{tikzpicture}[
        ...
        \node[anchor=east, bold] at (-0.1,-0.8) {Kodierung};
        \draw[line width=0.5mm, draw=red] 
            (-0.01,-1.1)--(0.5,-1.1)--(0.5,-0.5)--(0.75,-0.5)-- %1
            (0.75,-1.1)--(1,-1.1)--(1,-0.5)--(1.5,-0.5)-- %0
            (1.5,-1.1)--(1.75,-1.1)-- %1
            (1.75,-0.5)--(2,-0.5)--(2,-1.1)--(2.25,-1.1)-- %0
            (2.25,-0.5)--(2.5,-0.5)--(2.5,-1.1)--(3,-1.1)-- %0
            (3,-0.5)--(3.5,-0.5)-- %1
            (3.5,-1.1)--(4,-1.1)-- %1
            (4,-0.5)--(4.25,-0.5)-- %0
            (4.25,-1.1)--(4.5,-1.1)--(4.5,-0.5)--(4.75,-0.5);
    \end{tikzpicture}
\end{minipage}
...
\begin{center}
    TODO: Hier Abbildungen dazu einfügen
\end{center}
...
\begin{description}
    \item[Slow Start] Algorithmus zur Flusskontrolle bei TCP.
    \begin{enumerate}
    ...
    \end{enumerate}
    %
    \item[Schiebefensterprotokoll] Effizienzsteigerung, indem man $F$ viele Pakete hintereinander versendet, ohne auf die Bestätigung des vorherigen Pakets warten zu müssen. (\textit{Stop-and-Wait ist wie ein Schiebefensterprotokoll mit $F = 1$})
    \bigskip \\
...
\begin{description}
    ...
    %
    \item[DNS (\textit{Domain Name System})] System zum Auflösen von {\host}s zu IPs.
    \begin{itemize}
...
\begin{tabular}{l|l|l|l}
    Fehlersemantik & Bei Fehler & Filterung von Duplikaten & Bei Systemausfall \\ \hline
...
\begin{itemize}
    ...
    \item \textbf{C}onsistency: Transaktionen führen von konsistenten Zuständen wieder zu konsistenten Zuständen.
...
\begin{description}
    \item[2PC (\textit{2-Phase Commit Protocol})] Protokoll zur Umsetzung von \textit{ACID} in verteilten Systemen.
    \begin{enumerate}
        \item Voting-Phase:
        \begin{itemize}
...
\begin{description}
    \item[Zellulares Netzwerk] Das Frequenzband wird in unterschiedliche Teilfrequenzen aufgeteilt. Nachbarzellen nutzen andere Teilfrequenzen zur Vermeidung von Interferenzen. Das entstehende Muster (\textit{Cluster}) wird wiederholt.
    %
    \begin{itemize}
    ...
    \end{itemize}
    %
    \item[DHCP (\textit{Dynamic Host Configuration Protocol})] Protokoll zum automatischen Verteilen von IP-Adressen und Parametern wie DNS-Servern und Gateways.
    \begin{enumerate}
    ...
    \end{enumerate}
    %
    \item[Mobile IP] Protokoll um mobile Internetnutzer über eine statische IP adressieren zu können.
    \begin{itemize}

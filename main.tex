...
\begin{align*}
    &\textbf{SNR:} && \text{Signal-Rausch-Abstand (S-R-Verhältnis)} \hs{1cm} && \textbf{B:} && \text{Bandbreite} \\
    &\textbf{SR:} && \text{Symbolrate} \hs{1cm} && b \textbf{:} && \text{Bitrate} \\
    &\textbf{S:} && \text{Symbolstufen (Signalstufen)} \hs{1cm} && f_g \textbf{:} && \text{Grenzfrequenz} \\
...
\begin{tikzpicture}[
    gr/.style={line width=0.4mm, font=\bf}
]
    \draw[very thin,color=black!10] (-0.1,-0.1) grid (14.1,10.1);
    \draw[line width=0.8mm,->] (0,0)--(0,10) node[below left=3 and 0.5, fill=white] {\rotatebox{90}{Auslastung [\%]}};
    \foreach \y in {0,10,...,90} {
        \node at (-0.4,0.1*\y) {\y};
    };

    \draw[line width=0.8mm,->] (0,0)-- node[below=0.5] {Last bezogen Auf Kanaldatenrate [\%]} (14,0);
    ...
    \draw[gr] (0,0) .. controls (9,13) and (6,0) .. node[below right=1 and 0.8, align=center] {1-peristent\\CSMA} (12,0);
    \draw[gr] (0,0) .. controls (12,18) and (8,0) .. node[below right=2.5 and 1.1, align=center] {0.5-persistent\\CSMA} (13.5,0);
    \draw[gr] (0,0) .. controls (5,8) and (12,6) .. node[above right=0 and 3, align=center] {non-persistent\\CSMA} (14,5.5);
\end{tikzpicture}
